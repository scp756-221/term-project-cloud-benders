...
$(LOG_DIR)/dynamodb-init.log: cluster/cloudformationdynamodb.json
	...
	$(AWS) cloudformation create-stack --stack-name db-sh-ubh --template-body file://$< || true | tee $(LOG_DIR)/dynamodb-init.log
...
#
dynamodb-clean:
	$(AWS) cloudformation delete-stack --stack-name db-sh-ubh || true | tee $(LOG_DIR)/dynamodb-clean.log
...
s2v2: cluster/s2-dpl-v2.yaml cluster/s2-svc.yaml cluster/s2-sm.yaml cluster/s2-vs-canary.yaml
	$(KC) -n $(APP_NS) apply -f cluster/s2-dpl-v2.yaml | tee $(LOG_DIR)/s2.log
	$(KC) -n $(APP_NS) apply -f cluster/s2-svc.yaml | tee $(LOG_DIR)/s2.log
	$(KC) -n $(APP_NS) apply -f cluster/s2-sm.yaml | tee -a $(LOG_DIR)/s2.log
	$(KC) -n $(APP_NS) apply -f cluster/s2-vs-canary.yaml | tee -a $(LOG_DIR)/s2.log
...
s3v2: cluster/s3-dpl-v2.yaml cluster/s3-svc.yaml cluster/s3-sm.yaml cluster/s3-vs-canary.yaml
	$(KC) -n $(APP_NS) apply -f cluster/s3-dpl-v2.yaml | tee $(LOG_DIR)/s3.log
	$(KC) -n $(APP_NS) apply -f cluster/s3-svc.yaml | tee $(LOG_DIR)/s3.log
	$(KC) -n $(APP_NS) apply -f cluster/s3-sm.yaml | tee -a $(LOG_DIR)/s3.log
	$(KC) -n $(APP_NS) apply -f cluster/s3-vs-canary.yaml | tee -a $(LOG_DIR)/s3.log
...
cri: $(LOG_DIR)/s1.repo.log $(LOG_DIR)/s2.repo.log $(LOG_DIR)/s3.repo.log $(LOG_DIR)/db.repo.log $(LOG_DIR)/loader.repo.log
...
cr: registry-login
	...
	$(DK) push $(CREG)/$(REGID)/cmpt756s3:$(APP_VER_TAG) | tee $(LOG_DIR)/s3.repo.log
	$(DK) push $(CREG)/$(REGID)/cmpt756db:$(APP_VER_TAG) | tee $(LOG_DIR)/db.repo.log
	$(DK) push $(CREG)/$(REGID)/cmpt756loader:$(APP_VER_TAG) | tee $(LOG_DIR)/loader.repo.log
